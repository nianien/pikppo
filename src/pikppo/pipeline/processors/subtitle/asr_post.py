"""
ASR 后处理：切句、合并、speaker 处理、文本清理

职责：
- 从 ASR utterances 生成 segments（切句策略）
- speaker 处理
- 时间轴切分
- 语义切分（长句兜底）
- 最终文本清理（标点符号处理）
...
def words_to_segment(words: List[Word], speaker: str, full_text: Optional[str] = None) -> Segment:
    """
...
def split_utterance_axis_first(
    ...
    for w in words[1:]:
        ...
        # 1) speaker 变化：必切（硬边界）
        if w_spk != cur_spk:
            segments.append(words_to_segment(cur_words, cur_spk, full_text=utt.text))
            cur_words = [w]
            cur_spk = w_spk
            continue
