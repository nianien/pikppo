...
任何要给用户的提示，**按"空间范围"由大到小**走漏斗，第一个命中的就是答案——
不能随手挑渠道：

```
Q1  是影响全局的持续状态？        ──[是]──► 顶部 InfoBanner
Q2  发生在 Modal/Drawer 等局部容器内？──[是]──► 留在容器内（不外溢）
Q3  属于某一轮对话、需随历史留痕？  ──[是]──► 对话流内气泡（带交互/重试）
否则                                       ──────► 瞬时 Toast
```

- **顶部 InfoBanner —— 持续状态**：绑定一个布尔条件，条件为真常驻、为假自动消失，
  不由某次点击触发。例：模型未就绪 / 模型不可用（`modelService()==null`，含选了
  模型但无凭据）/ 离线 / MCP 未连。条件 = "**无可用模型服务**"，不要只判
  `currentModel.isEmpty`。
- **容器捕获性 —— 局部容器**：Modal/Drawer/弹窗里发生的事（释义/翻译/一键优化的
  结果与报错）**一律在容器内消化，绝不外溢**成 toast/banner。这条在 Q3 之前，避免
  "弹窗里触发了对话失败算 Q2 还是 Q3"的灰色地带。
- **对话流内气泡 —— 会话轮次（Turn，非仅"失败"）**：与某一轮对话绑定、需要留在历史
  且常带交互（重试）。范围比"错误"更广：`模型调用超时/报错`、`用户手动停止`、
  `需登录才能继续` 等非严格 error 的轮次状态都归这里。
- **瞬时 Toast —— 兜底**：离散动作的即时确认或无需留痕的轻反馈。例：已复制 / 已保存 /
  已删除 / 已收藏。

**跨层共存，但不跨层重复**：系统状态（Banner）与触发结果（对话气泡）可以**并存**——
例：断网时顶部 Banner 常驻"网络已断开"（全局环境），用户没注意又点发送，则该轮在
对话流里出"发送失败，请检查网络 + 重试"气泡（轮次结果）。两者在不同层、信息不冗余。
要避免的是**同一件事在同一层既 banner 又 toast**这种视觉轰炸。
